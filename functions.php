...
function loadEnrichmentDataFromCsv(string $filepath): array
{
    ...
    while (($row = fgetcsv($handle, 0, CSV_SEPARATOR, '"', '\\')) !== false) {
        ...
        if ($rawKey !== '')
            $data[(int) $rawKey] = ['ESITO' => trim((string) ($row[$idxEs] ?? '')), 'NOTA' => trim((string) ($row[$idxNo] ?? ''))];
    }
    fclose($handle);
    return $data;
}
...
function parseLine(string $line, array $schema): array
{
    ...
    foreach ($schema as $field) {
        if (!isset($field[REGOLA_POSIZIONE], $field[REGOLA_ALIAS], $field[REGOLA_LUNGHEZZA]))
            continue;
...
{
    if (!$schema || !is_readable($txtFilepath))
        return ['rows' => [], 'headers' => []];
...
function enrichRows(array &$rows, array $enrichmentData, array $schema): void
{
    ...
    foreach ($rows as &$row) {
        ...
        if ($key === '' || !isset($enrichmentData[(int) $key]))
            continue;
        if ($sEsito)
            $row['columns'][CAMPO_ESITO] = validateField($enrichmentData[(int) $key]['ESITO'] ?? '', $sEsito);
        if ($sNota)
            $row['columns'][CAMPO_NOTA] = validateField($enrichmentData[(int) $key]['NOTA'] ?? '', $sNota);
    }
    unset($row);
}
...
function handleExport(array $rows, array $schema): array
{
    ...
    $errors = [];
    $lines = [];
    $aliases = array_column($schema, REGOLA_ALIAS);
    $expLen = EXPECTED_LINE_LENGTH;
    foreach ($rows as $index => $row) {
        $line = '';
        foreach ($aliases as $alias)
            $line .= $row['columns'][$alias]['value'] ?? '';
        $len = strlen($line);
        if ($len !== $expLen)
            $errors[] = "Line " . ($index + 1) . ": Lunghezza invalida. Attesa {$expLen}, ottenuta {$len}.";
        else
            $lines[] = $line;
    }
    if ($errors)
    ...
    if (!is_dir($dir) && !mkdir($dir, 0777, true))
        return ['success' => false, 'errors' => ["Errore cartella: " . CARTELLA_MODIFICATI]];
    $filename = 'modificato_' . date('Ymd_His') . '.txt';
    if (file_put_contents($dir . DIRECTORY_SEPARATOR . $filename, implode("\n", $lines)) === false)
        return ['success' => false, 'errors' => ["Errore scrittura: $filename"]];
...
function handleDefinitiviExport(array $rows, array $schema): array
{
    ...
    if (!is_dir($dir) && !mkdir($dir, 0777, true))
        return ['success' => false, 'errors' => ["Errore cartella: " . CARTELLA_DEFINITIVI]];
...
function handleMerge(array $filesToMerge): array
{
    ...
    if (!is_dir($outDir) && !mkdir($outDir, 0777, true))
        return ['success' => false, 'errors' => ["Errore cartella: " . CARTELLA_UNITI]];
    $validFiles = [];
    foreach ($filesToMerge as $f) {
        $name = basename($f);
        $path = $inDir . DIRECTORY_SEPARATOR . $name;
        if (!is_file($path) || !is_readable($path))
            return ['success' => false, 'errors' => ["File danneggiato: $name"]];
        $validFiles[] = $path;
    }
    ...
    foreach ($validFiles as $file) {
        ...
        if (!$inHandle) {
            fclose($outHandle);
            return ['success' => false, 'errors' => ["Lettura fallita: " . basename($file)]];
        }
...
function deleteRowsByIds(array &$rows, array $idsToDelete, string $idType = CAMPO_ID_PRATICA): void
{
    ...
    foreach ($idsToDelete as $id) {
        $cleanId = ltrim(trim($id), '0');
        if ($cleanId === '')
            $cleanId = '0';
        $lookup[$cleanId] = true;
    }
    foreach ($rows as $key => $row) {
        $rawCell = (string) ($row['columns'][$idType]['value'] ?? '');
        $cleanCell = ltrim(trim($rawCell), '0');
        if ($cleanCell === '')
            $cleanCell = '0';
        if (isset($lookup[$cleanCell])) {
            unset($rows[$key]);
        }
    }
