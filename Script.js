document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyViewMode() {
        ...
        headers.forEach((th, index) => {
            if (index === 0) return; // Always keep the checkbox column
            
            // Note: We remove the sorting icon text when checking the name
    ...
    if (toggleSwitch) {
        ...
        applyViewMode(); // Apply immediately on load
    }

    // --- 2. Targeted Search Logic ---
    ...
    if (btnOpenManual && manualModal && btnCloseManual) {
        ...
        // Chiude la modale con il tasto ESC
